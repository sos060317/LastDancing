...
public class MeshDestroy : MonoBehaviour
{
        ...
        for (var i = 0; i < original.Triangles.Length; i++)
        {
            ...
            for (var j = 0; j < triangles.Length; j = j + 3)
            {
                ...
                if (sideCount == 2)
                {
                    //partMesh.AddTriangle(i,
                    //                     ray1.origin + ray1.direction.normalized * enter1,
                    //                     original.Vertices[transform[j + ((singleIndex + 1) % 3)]],
                    //                     original.Vertices[transform[j + ((singleIndex + 2) % 3)]],
                    //                     Vector3.Lerp(original.Normals[triangles[j + singleIndex]], original.Normals[triangles[j + ((singleIndex + 1) % 3)]], lerp1),
    ...
    public class PartMesh
    {
            ...
            Vector2 uv1, Vector2 uv2, Vector2 uv3)
        {
            ...
            Bounds.min = Vector3.Min(Bounds.min, uv1);
            Bounds.min = Vector3.Min(Bounds.min, uv2);
            Bounds.min = Vector3.Min(Bounds.min, uv3);
            Bounds.max = Vector3.Max(Bounds.max, uv1);
            Bounds.max = Vector3.Max(Bounds.max, uv2);
            Bounds.max = Vector3.Max(Bounds.max, uv3);
        }
        ...
        public void MakeGameobject(MeshDestroy original)
        {
            ...
            var filler = GameObject.AddComponent<MeshFilter>();
            filler.mesh = mesh;
